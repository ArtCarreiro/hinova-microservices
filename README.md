...
### Kafka (branch Kafka)
- Implementado em uma branch separada para demonstrar uma alternativa de comunicação assíncrona entre os serviços, utilizando Kafka como message broker.
  A comunicação síncrona via REST foi mantida na branch principal para simplicidade e clareza do fluxo, enquanto a branch Kafka serve como exemplo de como a arquitetura poderia evoluir 
  para um modelo mais desacoplado e resiliente.
...
- O uso de cache para otimizações de desempenho foi considerado fora do escopo desta implementação,
  uma vez que o projeto possui caráter demonstrativo e não exige otimizações avançadas neste estágio.
...
4. Assinar contrato no SIGN (`PUT /contracts/{uuid}/sign`).
